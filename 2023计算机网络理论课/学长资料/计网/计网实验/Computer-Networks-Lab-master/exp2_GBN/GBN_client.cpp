...
int main(int argc, char* argv[]) {
	...
	FILE* clientwrite = fopen("clientwrite.txt","w+");
	...
	int maxseqorder = 0;//定义当前收到的数据序号，避免重复写入文件
	while (true) {
		gets_s(buffer);
			...
			while (true) {
				...
				case 1://等待接收数据阶段
					...
					if (seq > maxseqorder&& seq<= SEQ_NUMBER) {
						//服务器端可能重传
						fwrite(&buffer[1], sizeof(char), strlen(buffer) - 1, clientwrite);
						maxseqorder = seq;
					}
					//随机法模拟包是否丢失
