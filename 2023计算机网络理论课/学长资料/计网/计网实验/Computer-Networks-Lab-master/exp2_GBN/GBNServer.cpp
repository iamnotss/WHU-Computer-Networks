...
const int SEQ_NUMBER = 17; // seq 的数目
...
int main(int argc, char* argv[]) {
	...
	FILE* serverwrite = fopen("serverwrite.txt", "w+");
		...
		if (strcmp(buffer, "-time") == 0) {
			...
			while (runFlag) {
				...
				case 2://数据传输阶段
					...
					/*为判断数据传输是否完成添加或修改的语句*/
					if (seqIsAvailable() && totalSeq <= (totalPacket - 1) && curSeq <= SEQ_NUMBER) {
						//发送给客户端的序列号从 1 开始
						buffer[0] = curSeq + 1;
						ack[curSeq] = FALSE;//将ACK变为待确认的状态
						//数据发送的过程中应该判断是否传输完成
						//为简化过程此处并未实现
					...
					if (recvSize == -1) {
						...
						/*为判断数据传输是否完成添加或修改的语句*/
						if (curAck - 1 == totalPacket) {//数据传输完成
							overflag = 1;
							printf("数据传输全部完成！！！\n");
		...
		/* 双向数据传输 */
			...
			while (true) {
				...
				case 1://等待接收数据阶段
					...
					if (seq > maxseqorder && seq <= SEQ_NUMBER) {
						//服务器端可能重传
						fwrite(&buffer[1], sizeof(char), strlen(buffer) - 1, serverwrite);
						maxseqorder = seq;
					}
					//随机法模拟包是否丢失
